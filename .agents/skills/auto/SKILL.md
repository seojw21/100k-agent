...
5. `.autopus/project/workspace.md` — root repo role, nested repo boundaries, tracking policy, QA target command routing
...
IMPORTANT: Do not wrap Autopus commands with GNU `timeout` (for example,
`timeout 540 auto spec review ...`). macOS does not ship GNU `timeout`, so that
wrapper fails with exit 127 before `auto` runs. For command deadlines, use the
Bash tool's native timeout/background controls. For Autopus provider execution
limits, pass `--timeout <seconds>` to `auto spec review` or `auto orchestra`.
If you see `command not found: timeout`, classify it as shell-wrapper failure
and rerun the underlying `auto ...` command without the wrapper.
...
| `--multi` | Risk-tiered provider review for code validation; high/critical changes fan out when providers are available. |
...
Agent(
  ...
  prompt = """
    ...
    - QA/Journey Pack target repositories and exact `auto qa init --project-dir <repo>` commands
  """
)
```
...
- `workspace.md` — root repo role, nested repo boundaries, generated/runtime paths, tracking/commit policy, source-of-truth locations, QA/Journey Pack target repos and `auto qa init --project-dir <repo>` commands
...
| `.autopus/project/workspace.md` | Root repo role, nested repo boundaries, generated/runtime paths, tracking policy, QA target command routing |
...
@.Codex/skills/autopus/planning.md
...
@.Codex/skills/autopus/tdd.md
@.Codex/skills/autopus/agent-pipeline.md
@.Codex/skills/autopus/worktree-isolation.md
...
IMPORTANT: This route uses Codex Agent Teams (experimental, disabled by default). Requires Codex v2.1.32+ AND `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`.

Preflight check (run before TeamCreate):
1. `Codex --version` — must be ≥ v2.1.32
2. `$CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` — must be `1`

If either check fails, abort with:
```
Agent Teams unavailable: Codex v2.1.32+ and CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 required.
...
Reference: `.Codex/skills/autopus/agent-teams.md`

##### [REQUIRED] Team Orchestration Procedure

IMPORTANT: The main (top-level) session owns ALL team lifecycle operations. `TeamCreate` automatically registers the main session itself as the `team-lead` member — do NOT spawn a separate Agent() for the lead role. Teammates cannot call `TeamCreate` (enforced by Codex at runtime).
...
**Side effect**: `~/.Codex/teams/{TEAM_NAME}/config.json` is created with the **main session already registered** as a member — `name: "team-lead"`, `agentType: <agent_type>`, `leadSessionId: <main-session-uuid>`. No separate `Agent(name="lead")` is needed; attempting one would create a duplicate and break the 4-member contract.
...
```bash
jq '.members | length' ~/.Codex/teams/{TEAM_NAME}/config.json
# expected: 4 (team-lead + builder-1 + tester + guardian)
jq -r '.members[].name' ~/.Codex/teams/{TEAM_NAME}/config.json
...
If `TeamDelete` returns an "active members" error, identify the remaining members via `jq -r '.members[].name' ~/.Codex/teams/{TEAM_NAME}/config.json`, re-send `shutdown_request` to each, and retry.

Worktree safety rules R1-R5 apply equally — each builder teammate works in an independent worktree. See @.Codex/skills/autopus/worktree-isolation.md and `.Codex/rules/autopus/worktree-safety.md`.
...
Ref: `.Codex/rules/autopus/context7-docs.md` for full detection heuristics, token limits, and anti-patterns. Ref: `.Codex/rules/autopus/techstack-freshness.md` for greenfield version evidence.
...
**GC Suppression**: During parallel worktree execution, all git commands MUST use `git -c gc.auto=0` to prevent garbage collection interference. See `.Codex/rules/autopus/worktree-safety.md`.

Migration directory serialization: tasks that create SQL migrations in the same owning repo and migration directory are sequential, even if their application code ownership is disjoint. Assign final migration numbers only after earlier migration branches are merged or rebased.

Parallel tasks (Mode = "parallel" AND no file ownership conflict AND no migration-directory conflict):
```
...
Sequential tasks (Mode = "sequential" OR file ownership conflict OR migration-directory conflict) do NOT use worktree isolation:
```
...
On lock error (`.git/refs.lock`, etc.), retry with exponential backoff: 3s → 6s → 12s (max 3 retries). See `.Codex/rules/autopus/worktree-safety.md`.
...
- Same owning repo + same SQL migration directory → force sequential until the previous migration branch is merged/rebased and the next number is recalculated
...
### Pipeline (--multi mode: risk-tiered provider review)

When `--multi` flag is set, activate the risk-tiered review policy:

- `low`/`medium` changes use single-provider review by default
- `high`/`critical` changes use `auto orchestra review` as a multi-provider dissent review when available
- If only one provider is installed, fall back to single-provider review and record degraded evidence instead of failing
- Can be combined with `--strategy`: consensus (default), debate, pipeline, fastest

`--team --multi` combination: Agent Teams + risk-tiered provider review activated in the Review Phase.
...
| `--team` | Agent Teams mode: use Codex Agent Teams (experimental) for multi-agent collaboration |
| `--solo` | Single session mode: no subagents, main session implements directly |
| `--multi` | Risk-tiered provider review: high/critical fan out when available, otherwise single-provider fallback |
...
@.Codex/skills/autopus/frontend-verify.md
...
@.Codex/skills/autopus/browser-automation.md
...
- Use `auto qa full --format json` as the easiest default entrypoint for full project QA planning. It inspects the release-style lane matrix, Journey Packs, setup gaps, and domain-readiness catalog without executing project commands. Add `--bootstrap` to create safe starter files first, and add `--run` only when the caller explicitly wants the full gate executed.
- Use `auto qa coverage --format json` after a run/release to summarize lane, journey, manifest, setup-gap, and domain-readiness coverage.
- Use `auto qa profile check --format json` when GUI/auth/profile capability readiness matters before execution.
...
- In a meta workspace, start with `auto qa full --format json`; if it returns project candidates, choose one with `--project-dir <repo>`. Do not write root `.autopus/qa/**` unless the root itself is the product repo.
...
- Do not edit generated root surfaces (`.Codex/**`, `.codex/**`, `.gemini/**`, `.opencode/**`, `.autopus/plugins/**`) directly.

Usage: `/auto qa full --format json`, `/auto qa full --bootstrap --format json`, `/auto qa full --run --format json`, `/auto qa coverage --format json`, `/auto qa profile check --format json`, `/auto qa init --format json`, `/auto qa init --local-only --format json`, `/auto qa plan --format json`, `/auto qa run --feedback-to codex --format json`, `/auto qa explore --dry-run --format json`, `/auto qa release --dry-run --format json`, `/auto qa release --roadmap --format json`, `/auto qa evidence --input manifest.json --output .autopus/qa/evidence --surface browser --lane golden --scenario S1`, `/auto qa feedback --to codex --evidence manifest.json`
...
@.Codex/skills/autopus/debugging.md
...
@.Codex/skills/autopus/review.md
...
@.Codex/skills/autopus/spec-review.md

Review a SPEC document using multiple providers (Codex, gemini, etc.) to validate quality. Delivers a PASS/REVISE/REJECT verdict via the Orchestra engine.
...
@.Codex/skills/autopus/security-audit.md
...
- `.autopus/project/workspace.md` — reflect root repo role, nested repo boundaries, generated/runtime paths, tracking/commit policy, QA/Journey Pack target command changes
